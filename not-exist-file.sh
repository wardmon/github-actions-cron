#ps -ef | grep http.server | awk '{print $2}'| xargs kill -9
ps -ef | grep server
#cat webserver.py
#nohup python3 -m http.server &
#cp bookmarklet-test.js.bak bookmarklet-test.bak.js
#cat bookmarklet-test.bak.js
#cp github-actions-cron/bookmarklet-test.js bookmarklet-test.js
#cat bookmarklet-test.js
#wget https://github.com/HACKERALERT/Picocrypt/releases/download/1.34/Picocrypt
#mv Picocrypt picocrypt
#chmod +x picocrypt
#cp picocrypt /usr/bin/picocrypt
#picocrypt
echo 'http://45.32.200.249:8000'
echo 'http://45.32.200.249:8060/ddd?url=ddd&ddd=ls -l;ps https://informedainews.com/ https://github.dev/wardmon/github-actions-cron'
echo 'result = subprocess.Popen(args='ls -l', shell=True, stdout=subprocess.PIPE)'
echo 'aa'
echo 'aa'
echo 'aa'
echo 'aa'
echo 'aa'
echo 'aa'
echo 'aa'
echo 'aa'
cat r.sh
date
echo 'hhttps://ward-myproj.pgs.sh/jsMind-intro%20(1).jmhhhhhha'\

cat *.txt
ls -lth
